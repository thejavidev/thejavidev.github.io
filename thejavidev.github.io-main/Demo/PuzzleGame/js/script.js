   ...
   var pieces = createPieces(true);
    ...
    function createPieces(withImage){
        ...
        var pieces = "";
            ...
            for(var i = 0,top=0,order=0; i<rows; i++,top-=100){
                for(var j=0,left=0; j<colums; j++,left-=100,order++){ 
                    if(withImage){
                        pieces += "<div style='background-position:"+ left + "px " 
                        + top + "px;' class ='piece' data-order=" + order + "></div>"
                    }
                    else{
    ...
    function checkIfPuzzleSolved(){
        ...
        for(var k = 0; k<16; k++){
            ...
            if( k != order){
                $("#pieceContainer").text("Ohh! Yeniden Basla");
                return false;
            }
        }
        $("#pieceContainer").text("Wow! Sen bir DAHİSİN");
        return true;
    }
        ...
        $(".droppableSpace").droppable({
           ...
           return !$(this).hasClass("piecePresent")
            ...
            drop: function(event,ui){
                var draggalbeElement = ui.draggable;
                var droppedOn = $(this);
                droppedOn.addClass("piecePresent");
                $(draggalbeElement).addClass("droppedPiece")
                .css({
                    top:0,
                    left:0,
                    position:"relative",

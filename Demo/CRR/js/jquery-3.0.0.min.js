...
!(function (a, b) {
				...
				function (a, b, e, f) {
					...
					if (((e = e || []), 1 === o.length)) {
						if (
							...
							if (
								((b = (d.find.ID(k.matches[0].replace(_, aa), b) ||
									[])[0]),
								!b)
							)
								return e;
				...
				e = {
					...
					then: function (b, d, e) {
						...
						return r
							.Deferred(function (a) {
								c[0][3].add(
									g(0, a, r.isFunction(e) ? e : M, a.notifyWith)
								),
			...
			if (
				...
				while ((h = o[g++]) && !b.isPropagationStopped())
					(b.type = g > 1 ? j : n.bindType || p),
						(m =
							(V.get(h, 'events') || {})[b.type] && V.get(h, 'handle')),
						m && m.apply(h, c),
						(m = k && h[k]),
						m &&
							m.apply &&
			...
			(l = r.event && o.global),
				...
				o.hasContent
					...
					: ((n = o.url.slice(f.length)),
					  o.data &&
							((f += (sb.test(f) ? '&' : '?') + o.data), delete o.data),
					  o.cache === !1 &&
							((f = f.replace(Ab, '')),
							(n = (sb.test(f) ? '&' : '?') + '_=' + rb++ + n)),
					  (o.url = f + n)),
				...
				g.length > 0 &&
					r
						...
						.done(function (a) {
							(f = arguments),
								g.html(
									d ? r('<div>').append(r.parseHTML(a)).find(d) : a
								);
						})
						.always(
							c &&
	...
	return (
		(r.noConflict = function (b) {
			return (
				a.$ === r && (a.$ = Ub), b && a.jQuery === r && (a.jQuery = Tb), r
			);
		}),
		b || (a.jQuery = a.$ = r),
		r
